(ns marketentry.marketentryllm
  ...
  the TZA public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-tz-entity?=" (:has-tz-entity? e)
                        " brela-verified?=" (:brela-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tz-entity? e))
                              (:has-tz-entity? e))
                          (or (not (:requires-brela? e))
                              (:brela-verified? e)))
                   0.9 0.3)}))
