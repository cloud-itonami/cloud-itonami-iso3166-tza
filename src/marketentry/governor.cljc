(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Tanzania-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Tanzania-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-tz-entity?
                                       true` (near-universal for Tanzania
                                       ...
                                       Grounded in Tanzanian legal entity
                                       (BRELA) typically required for public
                                       awards (contracting authorities / PPRA).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(BRELA/統一資格/TANePS/PPRA登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- tz-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-tz-entity? true`, INDEPENDENTLY verify
  `:has-tz-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-tz-entity?` ground truth (most Tanzania public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-tz-entity? e))
                 (not (true? (:has-tz-entity? e))))
        [{:rule :tz-entity-missing
          :detail (str subject " はタンザニア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- brela-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-brela? true`, INDEPENDENTLY check
  `:brela-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-brela? e))
                 (not (true? (:brela-verified? e))))
        [{:rule :brela-unverified
          :detail (str subject " はBRELA確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (tz-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (brela-unverified-violations request st)
