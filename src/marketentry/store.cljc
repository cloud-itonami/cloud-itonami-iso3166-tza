(ns marketentry.store
  "SSoT for the TZA market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems TZ" :portal "TANePS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-tz-entity? true :has-tz-entity? true
             :requires-brela? true :brela-verified? true
             :drafted? false :submitted? false
             :jurisdiction "TZA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "TANePS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-tz-entity? true :has-tz-entity? true
             :requires-brela? true :brela-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "TANePS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "TZA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "TANePS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-tz-entity? true :has-tz-entity? false
             :requires-brela? true :brela-verified? true
             :drafted? false :submitted? false
             :jurisdiction "TZA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "TANePS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-tz-entity? true :has-tz-entity? true
             :requires-brela? true :brela-verified? false
             :drafted? false :submitted? false
             :jurisdiction "TZA" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "TANePS/PPRA"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-tz-entity? has-tz-entity?
                               requires-brela? brela-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-tz-entity?)  (assoc :engagement/requires-tz-entity? requires-tz-entity?)
    (some? has-tz-entity?)       (assoc :engagement/has-tz-entity? has-tz-entity?)
    (some? requires-brela?)    (assoc :engagement/requires-brela? requires-brela?)
    (some? brela-verified?)    (assoc :engagement/brela-verified? brela-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-tz-entity? :engagement/has-tz-entity?
   :engagement/requires-brela? :engagement/brela-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-tz-entity? (boolean (:engagement/requires-tz-entity? m))
     :has-tz-entity? (boolean (:engagement/has-tz-entity? m))
     :requires-brela? (boolean (:engagement/requires-brela? m))
     :brela-verified? (boolean (:engagement/brela-verified? m))
